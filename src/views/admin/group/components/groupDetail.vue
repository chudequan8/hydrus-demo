  ...
  <el-row :gutter="20">
    <el-col :span="24">
      <el-button-group class="operation-group">
        ...
        >
          <i class="iconfont icon-authorityRole" />权限分配
        </el-button>
        <el-button
          type="primary"
          ...
          @click="handlerUser"
        >
          <i class="iconfont icon-relevance" />关联用户
        </el-button>
      </el-button-group>
    </el-col>
...
export default {
  ...
  created() {
    ...
    this.groupManager_btn_resourceManager = this.elements[
      'groupManager:btn_resourceManager'
    ]
  },
  computed: {
    ...mapGetters(['elements'])
  ...
  methods: {
    getList() {
      this.$api.group.fetchTree(this.listQuery).then(data => {
        this.treeData = data
      })
    },
    ...
    getNodeData(data) {
      ...
      this.$api.group.getObj(data.id).then(response => {
        this.form = response
      })
    ...
    handleDelete() {
      this.$confirm('此操作将永久删除, 是否继续?', '提示', {
        ...
        this.$api.group.delObj(this.currentId).then(() => {
          this.getList()
          this.resetForm()
          this.onCancel()
          this.$notify({
    ...
    update() {
      this.$api.group.putObj(this.form.id, this.form).then(() => {
        this.getList()
        this.$notify({
          title: '成功',
          message: '创建成功',
    ...
    create() {
      this.$api.group.addObj(this.form).then(() => {
        this.getList()
        this.$notify({
          title: '成功',
          message: '创建成功',
