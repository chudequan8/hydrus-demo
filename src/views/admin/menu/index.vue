...
export default {
  ...
  methods: {
    ...
    getNodeData(data) {
      ...
      this.$api.menu.getObj(data.id).then(response => {
        this.form = response
      })
    ...
    handleDelete() {
      this.$confirm('此操作将永久删除, 是否继续?', '提示', {
        ...
        this.$api.menu.delObj(this.currentId).then(() => {
          this.getList()
          this.resetForm()
          this.onCancel()
          this.$notify({
    ...
    update() {
      this.$api.menu.putObj(this.form.id, this.form).then(() => {
        this.getList()
        this.$notify({
          title: '成功',
          message: '更新成功',
    ...
    create() {
      this.$api.menu.addObj(this.form).then(() => {
        this.getList()
        this.$notify({
          title: '成功',
          message: '创建成功',
