...
export default {
  ...
  methods: {
    ...
    handleUpdate(row) {
      this.$api.element.getObj(row.id).then(response => {
        this.form = response
        this.dialogFormVisible = true
    ...
    handleDelete(row) {
      this.$confirm('此操作将永久删除, 是否继续?', '提示', {
        ...
        this.$api.element.delObj(row.id).then(() => {
          this.$notify({
            title: '成功',
            message: '删除成功',
            type: 'success',
    ...
    create(formName) {
      ...
      set[formName].validate(valid => {
        if (valid) {
          this.$api.element.addObj(this.form).then(() => {
    ...
    update(formName) {
      ...
      set[formName].validate(valid => {
        if (valid) {
          ...
          this.$api.element.putObj(this.form.id, this.form).then(() => {
            this.dialogFormVisible = false
            this.getList()
            this.$notify({
              title: '成功',
